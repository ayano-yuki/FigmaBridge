            ...
            try {
                ...
                if (node.type === 'GROUP') {
                    ...
                    nodeMetadata.groupStructure = {
                        id: groupNode.id,
                        name: groupNode.name,
                        children: ((_a = groupNode.children) === null || _a === void 0 ? void 0 : _a.map((child, index) => {
                            var _a;
                            const childInfo = {
                                id: child.id,
                                name: child.name,
                    ...
                    for (const childInfo of nodeMetadata.groupStructure.children) {
                        if (childInfo.fills && Array.isArray(childInfo.fills)) {
                            const imageFills = childInfo.fills.filter((fill) => fill.type === 'IMAGE');
                            if (imageFills.length > 0) {
                                // 子ノードのfillsにファイル名を追加
                                childInfo.fills = childInfo.fills.map((fill) => {
                                    if (fill.type === 'IMAGE') {
                                        // 子ノードの名前からファイル名を生成
                                        const childFileName = `${childInfo.name}.png`;
                                        const imageFillWithFileName = Object.assign(Object.assign({}, fill), { fileName: childFileName });
                ...
                if (node.parent && node.parent.type === 'GROUP') {
                    ...
                    nodeMetadata.groupIndex = ((_b = node.parent.children) === null || _b === void 0 ? void 0 : _b.indexOf(node)) || 0;
                }
                // フレーム内のノードの情報を追加
                if (node.parent && node.parent.type === 'FRAME') {
                    ...
                    nodeMetadata.frameIndex = ((_c = node.parent.children) === null || _c === void 0 ? void 0 : _c.indexOf(node)) || 0;
                }
                // コンポーネント内のノードの情報を追加
                if (node.parent && node.parent.type === 'COMPONENT') {
                    ...
                    nodeMetadata.componentIndex = ((_d = node.parent.children) === null || _d === void 0 ? void 0 : _d.indexOf(node)) || 0;
                }
                ...
                if (nodeMetadata.fills && Array.isArray(nodeMetadata.fills)) {
                    const imageFills = nodeMetadata.fills.filter((fill) => fill.type === 'IMAGE');
                    if (imageFills.length > 0) {
                        // 画像フィルにファイル名を追加
                        nodeMetadata.fills = nodeMetadata.fills.map((fill) => {
                            if (fill.type === 'IMAGE') {
                                const imageFillWithFileName = Object.assign(Object.assign({}, fill), { fileName: nodeMetadata.imageFileName });
                                return imageFillWithFileName;
                            }
                            return fill;
                        });
                ...
                if (node.type === 'FRAME' || node.type === 'GROUP') {
                    nodeMetadata.children = (_f = node.children) === null || _f === void 0 ? void 0 : _f.map(child => ({
                        id: child.id,
                        name: child.name,
                        type: child.type
                    }));
...
function processImageFiles(imageFiles) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            for (const imageFile of imageFiles) {
                try {
                    if (imageFile.data && imageFile.data.length > 0) {
                        ...
                        const image = yield figma.createImage(imageData);
            ...
            globalThis.imageDataMapping = imageDataMapping;
            globalThis.imageHashMapping = imageHashMapping;
            ...
            if (frameMetadata.fills && Array.isArray(frameMetadata.fills)) {
                ...
                const cleanFills = frameMetadata.fills.map((fill) => {
                    ...
                    if (fill.type === 'IMAGE' && fill.imageHash) {
                        ...
                        const imageDataMapping = globalThis.imageDataMapping;
                        const imageHashMapping = globalThis.imageHashMapping;
                        // ファイル名から画像データを取得
                        let imageData = null;
                        let imageHash = null;
                        if (fill.fileName && imageDataMapping && imageDataMapping[fill.fileName]) {
                            ...
                            if (fill.fileName && imageDataMapping) {
                                ...
                                // 完全一致を試行
                                let matchingFile = availableFiles.find(file => file === fill.fileName ||
                                    file.toLowerCase() === fill.fileName.toLowerCase());
                                // 完全一致が見つからない場合、部分一致を試行
                                if (!matchingFile) {
                                    matchingFile = availableFiles.find(file => file.includes(fill.fileName.replace('.png', '')) ||
...
function createGroupFromMetadata(groupMetadata) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            // グループの子ノード情報を取得
            const groupChildren = ((_a = groupMetadata.groupStructure) === null || _a === void 0 ? void 0 : _a.children) || [];
            // 子要素がない場合はグループを作成しない
            ...
            const maskIndex = groupChildren.findIndex((c) => c.isMask === true);
        ...
        try {
            ...
            if (textMetadata.textFills && Array.isArray(textMetadata.textFills)) {
                ...
                const cleanTextFills = textMetadata.textFills.map((fill) => {
                    ...
                    if (fill.type === 'IMAGE' && fill.imageHash) {
                        // 画像マッピングからハッシュを取得
                        const imageMapping = globalThis.imageMapping;
        ...
        try {
            ...
            if (childInfo.type === 'TEXT') {
                ...
                if (childInfo.textStrokes && Array.isArray(childInfo.textStrokes)) {
                    try {
                        const cleanTextStrokes = childInfo.textStrokes.map((stroke) => {
                            const cleanStroke = Object.assign({}, stroke);
                            // 認識されないプロパティを削除
                            delete cleanStroke.dashPattern;
                            return cleanStroke;
