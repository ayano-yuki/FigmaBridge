...
interface DesignNode {
  ...
  isMask?: boolean; // マスク情報を追加
  maskGroup?: boolean; // マスクグループ情報を追加
}
  ...
  private async serializeNode(node: SceneNode, depth = 0, parentType = '', parentX = 0, parentY = 0): Promise<DesignNode> {
    ...
    if ('children' in node && node.children) {
      ...
      for (const child of node.children) {
        if ('isMask' in child && (child as any).isMask) {
          hasMask = true;
          break;
        }
      }
      ...
      designNode.children = await Promise.all(node.children.map(async (child) => {
        const childNode = await this.serializeNode(child, depth + 1, node.type, node.x, node.y);
        if ('isMask' in child && (child as any).isMask) {
    ...
    if (nodeData.children && nodeData.children.length > 0) {
      ...
      if (nodeData.type === 'GROUP' || (node && node.type === 'FRAME')) {
        ...
        if (group.children.length > 0 && nodeData.children && nodeData.children[0].isMask && 'isMask' in group.children[0]) {
          (group.children[0] as any).isMask = true;
        }
        // maskGroup: trueのFRAMEやGROUPの場合はフレーム本体を生成し、その中にグループ化したマスク要素をappendChild
        if (nodeData.maskGroup && (nodeData.type === 'FRAME' || (nodeData.type as any) === 'GROUP')) {
          // 1. FRAMEノードを生成
          ...
          let groupNodes = Array.from(frame.children) as SceneNode[];
          // null/undefined除去
